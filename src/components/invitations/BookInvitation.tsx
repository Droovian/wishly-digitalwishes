...
import { OrbitControls, PerspectiveCamera, useTexture, Shadow, Text, Html } from '@react-three/drei'
      ...
      <animated.group rotation-y={rotation}>
        <mesh position={[0.5, 0, 0]} castShadow receiveShadow>
          ...
          />
          {
             (
                <>
                    <Text fontSize={0.1} color="black" position={[0, 0.5, 0.06]}>
                        Happy Birthday!
                    </Text>
                    <Text fontSize={0.05} color="black" position={[0, 0.1, 0.06]}>
                        Have an awesome day
                    </Text>
                </>
            )
